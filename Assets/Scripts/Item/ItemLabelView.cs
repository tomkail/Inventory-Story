...
public class ItemLabelView : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, ISlot, ISlottable {
    ...
    public ItemView itemView => GetComponentInParent<ItemView>(true);
    ...
    public IEnumerable<ISlot> GetEnterableSlots() {
        return GameController.Instance.sceneController.currentLevelController.itemViews.Select(x => x.labelView).Where(x => x != this).Concat(GameController.Instance.sceneController.currentLevelController.slotGroup.slots.Cast<ISlot>());
    }
    ...
    public void Start() {
        gameObject.name = $"Item Label View: {this.inkListItem.itemName}";
        labelLayout.textMeshPro.text = itemView.labelText;
        tooltip.tooltipText = itemView.tooltipText;
        Layout();
    ...
    void Update() {
        tooltip.enabled = levelController.draggingItemLabel == null;
        UpdateDrag();
    }
    void LateUpdate() {
    ...
    public void EnterSlot(ISlot slot) {
        ...
        } else if (containerSlot is ItemLabelView slottedItem) {
    ...
    public void OnPointerEnter(PointerEventData eventData) {
        if (levelController.draggingItemLabel != this) return;
        transform.SetAsLastSibling();
        hovered = true;
        UpdateSelectionState();
    }
    ...
    void UpdateLine() {
        Vector3 targetWorldPos = itemSpawnLocation.rectTransform.TransformPoint(itemSpawnLocation.rectTransform.rect.center);
        ...
        var labelScreenRect = layout.GetScreenRect();
        ...
        var startScreenPos = RectX.SplatVector(labelScreenRect, targetScreenPos-itemCenterScreenPos);
        
        var screenLineLength = Vector2.Distance(startScreenPos, targetScreenPos) * (labelScreenRect.Contains(targetScreenPos) ? -1 : 1);
    ...
    public Rect GetSlotTriggerScreenRectWhenContainingItem(ISlottable slottable) {
        if (!(slottable is ItemLabelView slottableItem)) return default;
