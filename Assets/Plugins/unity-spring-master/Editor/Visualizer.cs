...
namespace UnitySpring.Editor
{
    public class Visualizer : EditorWindow
    {
        ...
        bool usePhysicalParams;
        float damping;
        float mass;
        float stiffness;
        float dampingRatio = 0;
        float frequencyResponse = 0;
        ...
        void DrawController()
        {
            ...
            usePhysicalParams = EditorGUILayout.Toggle("Use Physical Params:", usePhysicalParams);
            if(usePhysicalParams) {
                // custom spring parameters
                BeginDisabledGroup(graphModeIndex == 0);
                BeginChangeCheck();
                {
                    ...
                    GUILayout.Space(10);
                    stiffness = EditorGUILayout.Slider("Stiffness:", stiffness, 0f, 800f);
                    GUILayout.Space(10);
                }
                if (EndChangeCheck()) UpdateCustomSpring();
                EndDisabledGroup();
            } else {
                // simplified spring parameters
                BeginDisabledGroup(graphModeIndex == 0);
                BeginChangeCheck();
                {
                    EditorGUIUtility.labelWidth = 120;
                    GUILayout.Space(10);
                    dampingRatio = EditorGUILayout.Slider("Damping ratio:", dampingRatio, 0f, 1f);
                    GUILayout.Space(10);
                    frequencyResponse = EditorGUILayout.Slider("Frequency response:", frequencyResponse, 0f, 1f);
                    GUILayout.Space(10);
                    
                    mass = 1f;
                    stiffness = Mathf.Pow(2 * Mathf.PI / frequencyResponse, 2) * mass;
                    damping = 4 * Mathf.PI * dampingRatio * mass / frequencyResponse;
                }
                if (EndChangeCheck()) UpdateCustomSpring();
                EndDisabledGroup();
            }
