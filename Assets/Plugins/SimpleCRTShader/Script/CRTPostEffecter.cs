...
public class CRTPostEffecter : MonoBehaviour
{
    ...
    private void OnRenderImage(RenderTexture src, RenderTexture dest)
    {
        ...
        //////Low resolution
        var target = RenderTexture.GetTemporary(Mathf.RoundToInt(screenHeight * (src.width / src.height)), screenHeight);
        Graphics.Blit(src, target);
        Graphics.Blit(target, dest, material);
        RenderTexture.ReleaseTemporary(target);
        // if (downscaling > 1)
        // {
        //     var target = RenderTexture.GetTemporary(Mathf.RoundToInt(src.width / downscaling), Mathf.RoundToInt(src.height / downscaling));
        //     Graphics.Blit(src, target);
        //     Graphics.Blit(target, dest, material);
        //     RenderTexture.ReleaseTemporary(target);
        // }
